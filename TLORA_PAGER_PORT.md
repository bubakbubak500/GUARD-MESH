> Upstream hardware port notes. For GUARD-MESH installation, use [local flashing](docs/LOCAL-FLASHER.md).
...
- **microSD is fully wired.** `CAP_SD`/`CAP_FILESYSTEM` are **1** — the staged
  ...
  replaced.
- **Both radio variants build locally.** Select the matching PlatformIO
  environment and generate a merged image for the local installer.
...
1. **Normal PlatformIO env, T-Deck model** — NOT the Tanmatsu IDF-subproject route.
   ...
   alongside the other board environments.
...
- [x] ⑨ **Local installation** — both Pager environments remain available.
  Build the matching LR1121 or SX1262 image and use the local GUARD-MESH installer.
  Upstream public release and deployment infrastructure has been removed.
